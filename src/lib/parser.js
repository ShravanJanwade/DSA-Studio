...
const SECTION_MAP = [
  // More specific first
  { keys: ['step by step solution', 'step-by-step solution', 'step by step approach', 'step-by-step approach', 'walkthrough', 'detailed solution', 'detailed approach', 'approach walkthrough'], field: 'in_depth_intuition' },
  { keys: ['in-depth intuition', 'in depth intuition', 'deep intuition', 'detailed intuition', 'intuition explained'], field: 'in_depth_intuition' },
  { keys: ['in-depth explanation', 'in depth explanation', 'deep explanation', 'detailed explanation', 'explanation in depth'], field: 'in_depth_explanation' },
  { keys: ['intuition', 'core idea', 'thought process', 'key insight', 'approach idea'], field: 'intuition' },
  { keys: ['description', 'problem statement', 'statement', 'problem description', 'what to do'], field: 'description' },
  { keys: ['visualization', 'dry run', 'visual', 'interactive', 'diagram', 'animation'], field: 'visualization_html' },
  { keys: ['algorithm', 'pseudocode', 'pseudo code', 'steps', 'procedure'], field: 'algorithm' },
  { keys: ['code', 'solution code', 'implementation', 'java solution', 'python solution'], field: 'code' },
  { keys: ['time complexity', 'time and space', 'complexity analysis', 'complexity'], field: '_complexity' },
...
const APPROACH_MAP = [
  { keys: ['brute force', 'brute', 'naive', 'straightforward', 'basic approach'], type: 'Brute Force' },
  { keys: ['better', 'improved', 'optimized approach', 'moderate'], type: 'Better' },
  { keys: ['optimal', 'efficient', 'best', 'tarjan', "o(n)", 'o(v+e)', 'o(v + e)'], type: 'Optimal' },
];
...
export function parseSingleProblem(content) {
  ...
  const result = {
    name: '', difficulty: 'Medium', description: '', in_depth_explanation: '',
    leetcode_url: '', gfg_url: '', youtube_url: '', solutions: [],
  };
  ...
  const gfgMatch = content.match(/https?:\/\/(www\.)?geeksforgeeks\.org\/[^\s)]+/);
  const ytMatch = content.match(/https?:\/\/(www\.)?(youtube\.com\/watch\?v=|youtu\.be\/)[^\s)]+/);
  ...
  if (diffMatch) result.difficulty = diffMatch[1].charAt(0).toUpperCase() + diffMatch[1].slice(1).toLowerCase();
  ...
  const flush = () => {
    ...
    if (!currentApproach) {
      ...
      else if (currentSection === 'in_depth_explanation') result.in_depth_explanation = text;
...
export function parseMultiProblem(content) {
  // ALWAYS try H1 splits first — these are the real problem boundaries
  // The regex excludes H1s that are clearly section headings, not problem names
  const sectionKeywords = /Brute|Better|Optimal|Intuition|Algorithm|Code|Hint|Description|Explanation|Step|Visualization|Complexity|In-depth|Walkthrough/i;

  const h1Pattern = /^# .+$/gm;
  const h1Matches = [...content.matchAll(h1Pattern)];

  // Filter to only H1s that look like problem titles (not section headings)
  const problemH1s = h1Matches.filter(m => !sectionKeywords.test(m[0]));

  if (problemH1s.length > 1) {
    // Split content at each problem H1
    const chunks = [];
    for (let i = 0; i < problemH1s.length; i++) {
      const start = problemH1s[i].index;
      const end = i + 1 < problemH1s.length ? problemH1s[i + 1].index : content.length;
...
export function parseExcel(workbook) {
  ...
  return rows.map(row => {
    const problem = {
      ...
      description: colMap(row, 'description', 'problemstatement', 'statement', 'desc'),
      in_depth_explanation: colMap(row, 'indepthexplanation', 'explanation', 'detailedexplanation', 'indepth'),
    ...
    for (const { prefix, type } of approaches) {
      const intuition = colMap(row, `${prefix}intuition`, `${prefix}_intuition`);
      const code = colMap(row, `${prefix}code`, `${prefix}_code`);
      if (intuition || code) {
        problem.solutions.push({
          approach_type: type,
          intuition,
          in_depth_intuition: colMap(row, `${prefix}indepthintuition`, `${prefix}_in_depth_intuition`, `${prefix}_step_by_step`, `${prefix}_walkthrough`),
          algorithm: colMap(row, `${prefix}algorithm`, `${prefix}_algorithm`, `${prefix}_algo`),
          code,
          language: colMap(row, `${prefix}language`, `${prefix}_lang`) || 'java',
          time_complexity: colMap(row, `${prefix}time`, `${prefix}_time_complexity`, `${prefix}timecomplexity`),
          space_complexity: colMap(row, `${prefix}space`, `${prefix}_space_complexity`, `${prefix}spacecomplexity`),
          visualization_html: colMap(row, `${prefix}visualization`, `${prefix}_visualization`, `${prefix}_viz`),
...
export async function parseUploadedFile(file) {
  ...
  // Count H1 headings that look like problem titles (not section headings)
  const sectionKw = /Brute|Better|Optimal|Intuition|Algorithm|Code|Hint|Description|Explanation|Step|Visualization|Complexity|In-depth|Walkthrough/i;
  const h1s = (text.match(/^# .+$/gm) || []).filter(h => !sectionKw.test(h));
